...
pair<pair<int,int>,pair<int,int>> point_encrypt(pair<int, int> G, int k, pair<int, int> Pm, pair<int, int> foreign_opened_key) // Шифрование точки
{
...
string encryption(string input, pair<int, int> G, int k, pair<int, int> foreign_opened_key) // Шифрование текста
{
	string result = "";
	...
	for (int i = 0; i < input.length(); i++)
	{
		...
		pair<pair<int, int>, pair<int, int>> pair_of_points = point_encrypt(G, k, it->second, foreign_opened_key); // Шифруем точку, получаем две
...
int main()
{
	...
	int my_closed_key = 7;
	pair<int, int> my_opened_key = mult(G, my_closed_key); // (27,22)
	pair<int, int> foreign_opened_key = make_pair(1, 13);
	int k = 5;
	int mode;
	...
	if (mode == 1)
	{
		...
		cin >> input;
		string encrypted = encryption(input, G, k, foreign_opened_key);
